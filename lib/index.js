  ...
  makeReduxModel(models) {
    models.forEach(model => {
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        (0, _invariant.default)(repeatModel.length <= 1, "model namespace should be unique, but got length = ".concat(repeatModel.length, ", with the same namespace is ").concat(model.namespace));
