...
var _autoAction = _interopRequireDefault(require("./autoAction"));
...
class RcReduxModel {
  ...
  start(models) {
    let _wrapRegisterAutoModels = models.map(model => {
      return (0, _autoAction.default)(model);
    });

    _wrapRegisterAutoModels.forEach(model => {
      // 1. 对每一个 model 都进行检查
    ...
    this.thunk = (0, _middleware.default)(_wrapRegisterAutoModels);
  }
  ...
  registerReducers(model) {
    ...
    (0, _invariant.default)(reducers, "model's reducers must be defined, but got undefined"); // 3.1 得到所有 reducers 中的 action

    const reducersActionTypes = Object.keys(reducers); // 3.2 reducers 是一个纯函数，function(state, action) {}

    return (storeState, storeAction) => {
      const newState = storeState || state; // 3.3 对 action 进行处理，规定 action.type 都是 namespace/actionName 的格式

      const reducersActionKeys = storeAction.type.split('/');
      const reducersActionModelName = reducersActionKeys[0];
      const reducersActionSelfName = reducersActionKeys[1]; // 3.3.1 如果不是当前的 model

      if (reducersActionModelName !== namespace) return newState; // 3.3.2 如果在 reducers 中存在这个 action
