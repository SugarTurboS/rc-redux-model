...
const autoSetStoreListAction = namespace => {
  return (_ref3) => {
    ...
    invariantAction(payloadType === '[object Array]', "[action][setStoreList] payload must be array");
    payload.map(s => {
      invariantAction(stateKeys.includes(s.key), "you didn't define the [".concat(s.key, "] in the model.state, please check for correctness"));
    });
    payload.map(s => {
      dispatch({
